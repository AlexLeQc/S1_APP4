				...
				int matrice2[MAX_HAUTEUR][MAX_LARGEUR],int *p_lignes2,int *p_colonnes2)
{
	...
	*p_colonnes2 = colonnes1;
	...
	return OK; //erreur de taille a ajouter peut etre (lignes et colonnes)
}

int pgm_creer_histogramme(int matrice[MAX_HAUTEUR][MAX_LARGEUR],
							int lignes, int colonnes, int histogramme[MAX_VALEUR+1]) //demander au prof pourquoi valeur max
{
	int valeur;
	for (int i = 0; i < lignes; i++)
	{
